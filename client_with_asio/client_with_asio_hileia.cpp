#include "client_with_asio_hileia.hpp"
#include <algorithm>
...
/* get system time */
void client_with_asio_hileia::itimeofday(long *sec, long *usec)
{
	struct timeval time;
...
IINT64 client_with_asio_hileia::iclock64(void)
{
    long s, u;
    IINT64 value;
    itimeofday(&s, &u);
...
IUINT32 client_with_asio_hileia::iclock()
{
    return (IUINT32)(iclock64() & 0xfffffffful);
}

std::string client_with_asio_hileia::get_milly_sec_time_str(void)
{
...
std::string client_with_asio_hileia::make_test_str(size_t test_str_size)
{
...
std::string client_with_asio_hileia::get_cur_time_str()
{
    time_t tmpcal_ptr = {0};
...
uint64_t client_with_asio_hileia::get_time_from_msg(const std::string& msg)
{
...
uint64_t client_with_asio_hileia::search_time_from_kcp_str(const std::string& kcp_str)
{
...
client_with_asio_hileia::client_with_asio_hileia(boost::asio::io_service& io_service, int udp_port_bind,
...
{
    kcp_client_hileia_.set_event_callback(client_event_callback_func, (void*)this);
    kcp_client_hileia_.connect_async(udp_port_bind, server_ip, server_port);
    kcp_client_hileia_.init_kcp(123456789, 1, 2, 20 ,1);
    hook_timer_send_msg();
}

void client_with_asio_hileia::stop_all()
{
  stopped_ = true;
}

void client_with_asio_hileia::send_test_msg(void)
{
...
void client_with_asio_hileia::print_recv_log(const std::string& msg)
{
...
void client_with_asio_hileia::hook_timer_send_msg(void)
{
    if (stopped_)
        return;
    client_timer_send_msg_.expires_from_now(boost::posix_time::milliseconds(SEND_TEST_MSG_INTERVAL));
    client_timer_send_msg_.async_wait(std::bind(&client_with_asio_hileia::handle_timer_send_msg, this));
}

void client_with_asio_hileia::handle_timer_send_msg(void)
{
    send_test_msg();
    hook_timer_send_msg();
}

void client_with_asio_hileia::client_event_callback_func(kcp_conv_t conv, eEventType event_type, kcp_frame& msg, void* var)
{
    ((client_with_asio_hileia*)var)->handle_client_event_callback(conv, event_type, msg);
}

void client_with_asio_hileia::handle_client_event_callback(kcp_conv_t conv, eEventType event_type, kcp_frame& msg)
{
    switch (event_type)
    {
        case eConnect:
