...
void client_with_asio::send_test_msg(void)
{
  ...
  kcp_client_.send_msg(test_str.c_str(), test_str.size());
}
...
void client_with_asio::client_event_callback_func(kcp_conv_t conv, eEventType event_type, kcp_buffer_data& msg, void* var)
{
    ((client_with_asio*)var)->handle_client_event_callback(conv, event_type, msg);
}

void client_with_asio::handle_client_event_callback(kcp_conv_t conv, eEventType event_type, kcp_buffer_data& msg)
{
    switch (event_type)
    {
        case eConnect:
